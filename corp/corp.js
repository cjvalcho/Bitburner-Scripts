/** @param {NS} ns **/
var corpName = "BitCorp"; /* An easy way to change the name of your corp and add flavor, 
rather than having to scour through the file and change things individually.*/

export async function main(ns) {
	ns.disableLog("disableLog"); ns.disableLog("sleep");
	
	if (!ns.getPlayer().hasCorporation) {
		ns.corporation.createCorporation(corpName + " Inc.");
	}
	var corp = ns.corporation.getCorporation();
	if (corp.divisions.length < 1) {
		// initial Company setup
		ns.corporation.expandIndustry("Tobacco", corpName + " Tobacco");
	...
	while (true) {
		...
		for (const division of corp.divisions.reverse()) {
			newProduct(ns, division); //Moved up to test if it's better up here.
		...
		for (const city of cities) {
			...
			if (ns.corporation.hasResearched(division.name, "Market-TA.II")) {
				...
				if (city == productCity) {
					...
					await ns.corporation.setAutoJobAssignment(division.name, city, "Business", Math.ceil(employees / 10));
					await ns.corporation.setAutoJobAssignment(division.name, city, "Management", Math.ceil(employees / 5));
					var remainingEmployees = employees - (3 * Math.ceil(employees / 5) + Math.ceil(employees / 10));
					await ns.corporation.setAutoJobAssignment(division.name, city, "Research & Development", Math.ceil(remainingEmployees));
				}
				else {
					await ns.corporation.setAutoJobAssignment(division.name, city, "Operations", Math.floor(employees / 5));
					await ns.corporation.setAutoJobAssignment(division.name, city, "Engineer", Math.floor(employees / 5));
					await ns.corporation.setAutoJobAssignment(division.name, city, "Business", Math.floor(employees / 50));
					await ns.corporation.setAutoJobAssignment(division.name, city, "Research & Development", Math.ceil(employees / 2));
					var remainingEmployees = employees - (2 * Math.floor(employees / 5) + Math.floor(employees / 50) + Math.ceil(employees / 2));
					await ns.corporation.setAutoJobAssignment(division.name, city, "Management", Math.floor(remainingEmployees));
...
function upgradeWarehouses(ns, division) {
	for (const city of cities) {
		...
		if (cityWarehouse.sizeUsed > 0.75 * cityWarehouse.size) {
...
async function trickInvest(ns, division, productCity = "Sector-12") {
	...
	ns.corporation.expandIndustry("Healthcare", corpName + " Medical Group");
	...
	for (var product of division.products) {
		if (ns.corporation.getProduct(division.name, product).developmentProgress < 100) {
			...
			if (ns.corporation.getProduct(division.name, product).sCost == 0) {
				...
				if (ns.corporation.hasResearched(division.name, "Market-TA.II")) {
					//ns.corporation.setProductMarketTA1(division.name, product, true); //No reason to do this
					ns.corporation.setProductMarketTA2(division.name, product, true);
				}
			}
		}
