...
export async function main(ns) {
	...
	function getUpgradeInfo(index, stats, field) {
		...
		newStats[field] += 1;
		const gain = hacknetServers.hashGainRate(newStats.level, 0, newStats.ram, newStats.cores, playerMulti);
		//ns.tprint("Upgrade Gain: " + gain); //added by me
		const roi = gain / cost;
		//ns.tprint("Upgrade ROI: " + roi); //added by me
		return {
			cost,
			roi,
			gain
	...
	function getPurchaseInfo() {
		...
		const gain = hacknetServers.hashGainRate(1, 0, 1, 1, playerMulti);
		//ns.tprint("Purchase Gain: " + gain); //added by me
		const roi = gain / cost;
		//ns.tprint("Purchase ROI: " + roi); //added by me
		return {
			cost,
			roi,
			gain
	...
	async function waitForMoney(cost) {
		while ((ns.getServerMoneyAvailable("home") * 0.5) < cost) {
	...
	while (true) {
		...
		//await ns.sleep(10000); //added by me
